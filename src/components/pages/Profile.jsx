...
const Profile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const res = await axios.get(`${import.meta.env.VITE_AUTH_BACKEND_URL}/details`, {
    ...
    <div className="px-6 flex justify-center items-center py-10 text-gray-100">
      <div className="relative w-full bg-gray-900 rounded-2xl shadow-2xl border border-yellow-500/20 p-8 overflow-hidden">
        ...
        <div className="relative flex flex-col items-center text-center mb-8">
          ...
          <h1 className="mt-4 text-3xl font-bold text-yellow-400">{driver.name}</h1>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mb-8">
          ...
          <InfoCard label="Phone" value={driver.phoneNumber} icon="pi pi-phone" />
          <InfoCard label="License No" value={driver.licenseNumber} icon="pi pi-id-card" />
          <InfoCard label="Aadhar No" value={driver.aadharCardNumber} icon="pi pi-credit-card" />
          <InfoCard label="Approval" value={driver.driverApprovalStatus} icon="pi pi-check-circle" />
          <InfoCard label="Rating" value={`${driver.rating} ★`} icon="pi pi-star" />
          <InfoCard label="Active Booking" value={driver.activeBooking} icon="pi pi-car" />
          <InfoCard label="City" value={driver.activeCity || "N/A"} icon="pi pi-map-marker" />
        </div>
